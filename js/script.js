    ...
    $(document).keydown(function(objEvent) {
        if (objEvent.ctrlKey) {
			if (objEvent.shiftKey) {
				if (objEvent.keyCode == 73) { return enableKeys; } // Disable CTRL + SHIFT + I
				else if (objEvent.keyCode == 74) { return enableKeys; } // Disable CTRL + SHIFT + J
			} else {
				if (objEvent.keyCode == 65) { return enableKeys; } // Disable CTRL + A
				else if (objEvent.keyCode == 85) { return enableKeys; } // Disable CTRL + U
			}
        }
	});
});
...
const commands = {
	aboutme: {
		cmd: 'aboutme',
		res: 'Bio:<br># <span class="red">Coding:</span> HTML, CSS, JS, PHP, C++, Go.<br># <span class="red">Languages:</span><br><br>- English <span class="red">(Perfect)</span><br>- French <span class="red">(Almost perfect)</span><br>- Greek <span class="red">(Mother tongue)</span><br><br>- Swedish <span class="red">(Fluently)</span>'
	},
	links: {
		cmd: 'links',
		res: 'Links:<br># <span class="red"><a href="https://steamcommunity.com/id/atlant1cs" target="_blank"><u>Steam</u></a></span> - One of my accounts, mostly use this one.<br># <span class="red"><a href="https://twitter.com/sw4gl0rds4uc3" target="_blank"><u>Twitter</u></a></span><br># <span class="red"><a href="https://www.youtube.com/channel/UCVdvONJWGlkg2w_j1sHmeCQ?view_as=subscriber" target="_blank"><u>Youtube</u></a></span>'
	},
	visitors: {
		cmd: 'visitors',
	...
	help: {
		cmd: 'help',
		res: 'Available commands:<br><br># <span class="red">aboutme</span> - Some information about me.<br># <span class="red">clear</span> - Cleans the command line. <br># <span class="red">screenfetch</span> - Fetches system information in the terminal.<br># <span class="red">links</span> - Links for others sites.<br># <span class="red">whoami</span> - Info about you.'
	}
}

var prephraseRoot = '<span class="red">root@pwned.xyz</span>:<span class="blue">~</span># ';
var prephrase = '<span class="red">visitor@pwned.xyz</span>:<span class="blue">~</span># ';
...
function launchCommandAsRoot(command) {
	...
	if (command === './welcome.sh')
		res = 'Welcome at <span class="red">pwned.xyz</span>!<br><br>You\'re visiting from: <span class="red">' + visitorCountry + '</span><br>Last 5 Country Visits: <span class="red">' + lastCountry0 + '</span>, <span class="red">' + lastCountry1 + '</span>, <span class="red">' + lastCountry2 + '</span>, <span class="red">' + lastCountry3 + '</span>, <span class="red">' + lastCountry4 + '</span>.<br>Total Visits: <span class="red">' + visits + '</span><br>Terminal Updated: <span class="red">' + lastUpdate +'</span><br>Forum registrations are currently disabled.<a href=".f/" class="red"></a><br><br><b>Type \'help\' for more details on the commands</b>'
	else
	{
...
$('form').on('submit', function(e) {
	...
	try {
			...
			if (vInput.startsWith("./")) {
				if (vInput === "./root/welcome.sh") {
					...
					launchCommandAsVisitor({
						cmd: vInput,
						res: '<div><p>pwned.xyz: ' + vInput + ': No such file or directory</p></div>'
					});
				}
				...
				if (typeof loginArray[2] !== "undefined") {
					if (loginArray[2] === "visitor") {
						...
						res = 'pwned.xyz: No passwd entry for user ' + loginArray[2] + '';
					}
					launchCommandAsVisitor({
				...
				launchCommandAsVisitor({
						cmd: $('input').val(),
						res: '<div><p><span class="red">visitor@pwned.xyz</span><br><span class="red">OS:</span> pwned.xyz ' + OSVer + '<br><span class="red">Kernel:</span> ' + Kernel + '<br><span class="red">Uptime:</span> ' + upTime + '<br><span class="red">Shell:</span> ' + Shell + '<br><span class="red">Disk:</span> ' + Disk + '</p></div>'
					});
			} else if (vInput.trim() !== "") {
		...
		if (root > 0) {
			launchCommandAsRoot({
				cmd: $('input').val(),
				res: '<div><p>pwned.xyz: ' + $('input').val() + ': command not found</p></div>'
			});
		} else {
			launchCommandAsVisitor({
				cmd: $('input').val(),
